...
#define PROP_SUBSYS_STATE_PATH "ro.vendor.subsys_state_notifier.%s.state_path"

int main(int argc, char **argv) {
    char prop_key[64];
    char subsystem[10];
    char subsystem_state[11];
    char subsystem_state_path[PROPERTY_VALUE_MAX];
    int fd;
    ...
    // Get subsystem state path from property
    snprintf(prop_key, sizeof(prop_key), PROP_SUBSYS_STATE_PATH, subsystem);
    property_get(prop_key, subsystem_state_path, "");
    if (!strlen(subsystem_state_path)) {
        ALOGE("%s subsystem state path is invalid\n", subsystem);
        goto error;
    }
    ...
    if (fd < 0) {
        ALOGE("Failed to open %s subsystem state path %s\n", subsystem, subsystem_state_path);
        goto error;
    }

    // Set property key for notifying state
    snprintf(prop_key, sizeof(prop_key), PROP_SUBSYS_STATE, subsystem);
    ...
    while (true) {
        ...
        property_set(prop_key, subsystem_state);
