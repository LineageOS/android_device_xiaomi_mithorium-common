...
#define LOG_INFO(...)  fprintf(stderr, LOG_TAG __VA_ARGS__)
...
int active_led_brightness = 0;
int active_led_id = LED_UNKNOWN;
int active_led_state = LED_STATE_OFF;
...
bool stop_active_led(void) {
    ...
    switch (active_led_state) {
        case LED_STATE_BLINK:
            succeed = set_led_breath(active_led_id, false);
            break;
        case LED_STATE_BRIGHTNESS:
            succeed = set_led_brightness(active_led_id, 0);
            break;
        case LED_STATE_OFF:
            succeed = true;
            break;
    ...
    if (succeed)
        active_led_state = LED_STATE_OFF;
...
bool update_active_led_brightness(void) {
    ...
    if (active_led_state != LED_STATE_BRIGHTNESS ||
        active_led_brightness == cur_led_brightness)
        return true;

    succeed = set_led_brightness(active_led_id, cur_led_brightness);
    if (succeed)
        active_led_brightness = cur_led_brightness;
...
bool update_led(int led_id, int led_state) {
    ...
    if (active_led_id == led_id && active_led_state == led_state) {
        return true;
    }
...
void handle_battery_capacity_update_white(void) {
    if (bat_capacity <= 10) // 0 ~ 10
        update_led(active_led_id, LED_STATE_BLINK);
    else if (bat_capacity <= 99) // 11 ~ 99
        update_led(active_led_id, LED_STATE_BRIGHTNESS);
    else if (bat_capacity == 100) // 100
        update_led(active_led_id, LED_STATE_BLINK);
}
...
int main(int argc, char **argv) {
    ...
    // Use blink node?
    if (active_led_id == LED_UNKNOWN) {
        // RGB LED
        if (file_is_writeable(led_blink_paths[RED]))
            use_blink_node = true;
    } else {
        // White LED
        if (file_is_writeable(led_blink_paths[active_led_id]))
            use_blink_node = true;
    }
    if (use_blink_node)
