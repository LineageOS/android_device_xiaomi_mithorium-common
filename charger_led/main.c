...
bool set_led_brightness(enum led_types led_id, bool enable) {
    ...
    if (!enable)
        return write_led_brightness(led_id, 0);
...
bool set_led_state(enum led_types led_id, enum led_states led_state) {
    ...
    if (!succeed) {
        LOG_ERROR("Failed to set %s LED state to %s\n", led_id_to_str(led_id), led_state_to_str(led_state));
    }
    return succeed;
}
...
void handle_battery_capacity_update_rgb(void) {
    ...
    else if (bat_capacity <= 30)  // 11 ~ 20
        update_led(RED, LED_STATE_BRIGHTNESS);
#ifdef __ANDROID_RECOVERY__
    else if (bat_capacity <= 60)  // 31 ~ 60
...
int main(void) {
    ...
    // Event loop
    while (true) {
        if (read_fd_to_int(usb_online_fd, &usb_online)) {
            if (!usb_online) {
#ifdef DEBUG
